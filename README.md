...
> [!IMPORTANT]
> - **NOLoader required** - use the **NOLoader** release zip (`MpClientOpt-v*-NOLoader.zip`) unless you explicitly run the BepInEx mirror workflow.
> - **PatchTool required on first install** - 58 IL patches in `mod.json`; run `deploy-mp-opt-mod.ps1` with game **closed**.

> [!WARNING]
> - **Dedicated client only** - mod disables itself when `Server.Active == true` (host / listen-server).
> - **Do not disable `memory_budget` on dedicated clients** - `memory_budget=0` caused significant FPS loss in testing.
> - **Avoid v0.6.3 and v0.6.4 builds** - known FPS regressions; use **v0.6.5+**.
> - **Do not combine RAM reservoir with NORAMAdjust** - see [NORAMAdjust](https://github.com/Mursisru/NORAMAdjust) compatibility notes.
...
**Stable release:** `v0.6.9` тАФ full documentation refresh. Code matches `v0.6.8` (map icon sync, missile salvo, presentation physics fixes). FPS core unchanged from verified `v0.6.2`.
...
- [NOLoader](https://github.com/Mursisru/NOLoader) тАФ primary loader ([install guide](https://github.com/Mursisru/NOLoader/blob/main/docs/INSTALL.md))
...
4. **First install only** тАФ close the game, then from a machine with `NOLoader_Engine` built:
...
|------|-----|-----------|-------|
| Any `v0.6.2`тАУ`v0.6.9` | New `v0.6.x` | **Not required** if IL patches already applied | Replace DLL + `mod.json` + `mod.ini` |
...
C#-only fixes (`v0.6.6`тАУ`v0.6.8`) do not change the 58 IL patch manifest.
...
| `presentation_far_m` | 4000 | Combat zone тАФ VisualUpdate never skipped inside |
...
```text
0 тФАтФАтФАтФАтФАтФАтФАтФАтФА presentation_full_m (400m) тФАтФАтФАтФАтФАтФАтФАтФАтФА presentation_near_m (800m)
      тФВ full zone (stride/offscreen)          тФВ near/mid (VisualUpdate stride)
      тФФтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА presentation_far_m (4000m) тФАтФАтФА combat zone тФАтФАтФАтФР
                                                                       тФВ
                                              deep_freeze_min_m (40km) тФШ
```
...
| Own missiles | `MpPresentationExemptGuard` | Owner = local aircraft тЖТ exempt from cull/freeze |
...
| **Green** | Remote aircraft components (gear, surfaces, pilot, fuel) тАФ distance tiered |
...
| **Yellow** | RB throttle тАФ transform-only `ApplySnapshot` + velocity/`rb.Move` (missiles always vanilla) |
...
- Distance тЙд `deep_freeze_min_m`
- In your target list
...
|---------|--------------|--------|
| Map icon тЙа 3D model | Old build before `v0.6.8` | Upgrade to `v0.6.8+` |
...
Logs: `Nuclear Option\NOLoader\logs\noloader_ring.log` тАФ search `[MpOpt]`.
...
`v0.6.3` and `v0.6.4` regressed FPS тАФ **not shipped**. Do not re-enable their experimental ini/IL patterns without A/B testing.
